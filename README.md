...
Sources/SemanticResponseCache/           (the library — no UI, no app)
  ...
  Saturating.swift           non-trapping add/multiply (used for every counter and byte total) plus `int(_:)` / `divide` helpers for callers
...
**4. Coverage-aware eviction, with LRU shipped alongside as the control.** LRU evicts the least-recently-hit entry. `CoverageAwareEviction` evicts the entry whose nearest neighbour in the cache is closest to it — the one whose removal loses the least coverage of the prompt space — with ties on fewest hits, then age, then id, so it is total and deterministic. The suite constructs a case where the two policies *must* disagree (two recent near-duplicates and one old singleton) and asserts that they do, and a second test replays the same interleaved trace under a budget of 6 and asserts coverage-aware's second-pass hit rate beats LRU's. On the bundled corpus (threshold 0.50, shadow sampling off, second pass of the 30-prompt trace), budget 6: **LRU 3/30 vs coverage-aware 14/30**; budget 16: **18/30 vs 27/30**. With shadow sampling at 100% the budget-6 coverage figure is 10/30 — the self-healing inserts change what is in the cache — which is why the condition is stated. *Rejected:* LFU — hit count is a proxy for coverage, and a worse one; centroid clustering — right idea, more machinery than the entry counts justify.
...
No force-unwraps. Every `[i]` is inside a bounds-guarded loop. `Embedding.init(normalizing:)` refuses NaN, infinity, zero norm and empty input, so no degenerate vector can enter the cache. `cosine` returns `nil` on a dimension mismatch rather than reading out of bounds. All counters and byte totals go through `Saturating.add` / `Saturating.multiply`; the one `%` in the package (bucket selection in the embedder) is guarded by the constructor's `dimension >= 8` check. `CachePolicy`, `ShadowConfiguration` and `HashedTrigramEmbedder` throw on invalid parameters at construction, so an invalid configuration cannot reach a request. `Int(Double)` appears nowhere unguarded.

## The embedder is honest

`HashedTrigramEmbedder` is feature hashing over words and padded character trigrams, L2-normalised. It is **deterministic, dependency-free, and runs on Linux**, which is what lets the entire threshold / eviction / staleness / shadow-sampling machinery be exercised on real geometry in CI. It is **not a language model**: it has no notion of synonymy, and on the bundled corpus the same-topic paraphrase similarities run **0.44–0.87** for paraphrases that differ after normalization (one paraphrase normalizes identically to its canonical prompt — that is an exact-tier hit, never a vector comparison) while the worst cross-topic pair sits at **0.55**. Two consequences the package owns rather than hides:

- **The server-side 0.92 is a *semantic* hit rate of zero with this embedder** — the exact tier still answers verbatim repeats (on the demo's second pass at 0.92: 6 exact hits, 0 semantic, 10% overall). The demo lets you set it and watch the SEMANTIC badges disappear. The right threshold is a property of the embedder, which is precisely why the shadow sampler exists.
- **There is no threshold on this corpus with both 100% paraphrase recall and 0 false hits.** Replaying the bundled corpus twice through `ReplayRunner` with shadow sampling at 100%: at 0.45 it measures **3 disagreements in 27 shadow samples** (Wilson upper bound ≈ 28%; the advisor still says `insufficientEvidence(27, needed: 30)` because it refuses to recommend on fewer than 30 samples — a third pass crosses that bar and it says *raise*); at 0.50 it measures **0 in 24**. That overlap is the actual design problem, and the instrument that measures it is the deliverable.
